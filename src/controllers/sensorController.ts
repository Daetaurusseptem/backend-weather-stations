...
export const obtenerDatoSensorPorEstacionId = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const datoSensor = await SensorData.findOne({ estacion: estacionId })
      .sort({ timestamp: -1 })
      .populate('estacion');
